...
## Regular repeating function call

TODO: once figured out if repeating thread is performant effective

## logging an event from the UI

TODO: This should be in the experiment helpers not in the metrics

```python
with gui.Col(pos=(270, 10)) as col:
    col.add(gui.Text_Input(label='log event', on_enter=get.log_event))
```
...
## Spreading the Visual across multiple displays
...
## set a peripheral like a valve to a specific state upon the task end using run_at_quit

## switching parameters or blocks based on performance

## Add a configuration for a different setup whose addtional devices are ignored in the first setup
